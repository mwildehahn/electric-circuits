...
| `ELECTRIC_CIRCUITS_DS_READ_MAX_BYTES` | `67108864` (64 MiB) | Durable Streams: client-side hard cap for any response body; exceeding it fails the read with the stream path and observed size |
...
| `ELECTRIC_CIRCUITS_TXN_MEMORY_BYTES` | `33554432` (32 MiB) | Large transactions: in-memory bytes of ONE transaction the ingestor may buffer before it spills the rest to disk. `0` = never spill (buffer the whole transaction in RAM) |
